...
function Hero({ locale }: { locale: Locale }) {
  return (
    <section className="border-line relative isolate flex min-h-screen overflow-hidden border-b">
            ...
            <div className="absolute right-0 top-[calc(100%+10px)] w-56 overflow-hidden rounded-[20px] border border-black/5 bg-white p-2 text-left shadow-[0_18px_50px_rgb(15,23,42,0.14)]">
              <nav className="space-y-1">
                {NAV_ITEMS.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="text-fg-muted hover:text-fg block rounded-[14px] px-3 py-2 text-[13px] font-medium transition hover:bg-black/[0.04]"
                  >
                    {item.label}
                  </Link>
                ))}
              ...
              <div className="border-line mt-2 border-t pt-2">
                <Show when="signed-out">
                  ...
                  <Link
                    href="/signup"
                    className="bg-fg text-bg hover:bg-fg-muted mt-1 block rounded-[14px] px-3 py-2 text-center text-[13px] font-medium transition"
                  >
                    Start signup
                  </Link>
                </Show>
                <Show when="signed-in">
                  <Link
                    href="/overview"
                    className="bg-fg text-bg hover:bg-fg-muted block rounded-[14px] px-3 py-2 text-center text-[13px] font-medium transition"
                  >
                    Dashboard
                  </Link>
                </Show>
