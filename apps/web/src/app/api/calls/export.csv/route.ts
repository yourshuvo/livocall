...
export async function GET(req: Request) {
  ...
  const header = [
    ...
    'businessOutcomeConversion',
    'businessOutcomeConfidence',
    'businessOutcomeAmountPaisa',
    'businessOutcomeCallbackAt',
    'businessOutcomeCallbackE164',
  ...
  const rows = calls.map((c) =>
    [
      ...
      c.tier,
      c.outcome,
      c.businessOutcome?.key ?? '',
      c.businessOutcome?.label ?? '',
      c.businessOutcome?.conversion ?? '',
      c.businessOutcome?.confidence ?? '',
      c.businessOutcome?.amountPaisa ?? '',
      c.businessOutcome?.callbackAt ? new Date(c.businessOutcome.callbackAt).toISOString() : '',
      c.businessOutcome?.callbackE164 ?? '',
      c.businessOutcome?.notes ?? '',
      c.durationSec ?? 0,
      c.cost?.totalPaisa ?? 0,
      c.summary ?? '',
    ]
