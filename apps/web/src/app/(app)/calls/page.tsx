    ...
    <>
      ...
      <div className="grid gap-4 px-6 pt-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryCard
          ...
          hint={
            totals.count
              ? `${Math.round((totals.completed / totals.count) * 100)}% rate`
              : '—'
          }
        />
        <SummaryCard
          ...
          hint="aggregate"
        />
        <SummaryCard label="Spend" value={fmtBdt(totals.spend)} icon="currency" hint="paisa-precise" />
      </div>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-1.5">
          {FILTERS.map((f) => {
            ...
            return (
              <Link
                key={f.key}
                href={href + (q ? `${f.key === 'all' ? '?' : '&'}q=${encodeURIComponent(q)}` : '')}
                className={cn(
              ...
              >
                {f.tone === 'live' && <span className="size-1.5 rounded-full bg-status-live" />}
                {f.tone === 'warn' && <span className="size-1.5 rounded-full bg-status-warn" />}
                {f.tone === 'fail' && <span className="size-1.5 rounded-full bg-status-fail" />}
                {f.label}
              </Link>
            )
          })}
          <form method="get" action="/calls" className="ml-auto flex items-center gap-2">
            ...
            <input
              ...
              className="rounded-full border border-line bg-bg px-3 py-1 font-mono text-[11px] text-fg placeholder:text-fg-faint focus:border-fg/30 focus:outline-none"
            />
          </form>
        </div>
          ...
          {calls.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-bg-subtle/70 text-[11px] uppercase tracking-[0.08em] text-fg-muted">
                  <tr>
                ...
                <tbody>
                  {calls.map((c) => (
                    <tr
                      key={String(c._id)}
                      className="cursor-pointer border-t border-line transition hover:bg-bg-subtle/40"
                    >
                      ...
                      <td className="px-5 py-3 font-mono text-xs">
                        <Link href={`/calls/${c._id}`}>
                          ...
                          <Icon name="arrow-right" size="xs" className="mx-1.5 text-fg-faint" />
                      ...
                      <td className="px-5 py-3 text-right">
                        <Link
                          href={`/calls/${c._id}`}
                          className="inline-flex size-7 items-center justify-center rounded-md border border-line bg-bg text-fg-muted transition hover:border-fg/30 hover:text-fg"
    ...
    <Card className="relative overflow-hidden">
      ...
      <CardBody className="relative flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <span className="grid size-7 place-items-center rounded-md border border-line bg-bg-subtle text-fg-muted">
            <Icon name={icon} size="sm" />
          </span>
        </div>
        <p className="font-display text-[24px] font-medium tracking-tightest text-fg">{value}</p>
        <p className="font-mono text-[10.5px] uppercase tracking-[0.12em] text-fg-faint">{hint}</p>
      </CardBody>
    </Card>
  )
}
